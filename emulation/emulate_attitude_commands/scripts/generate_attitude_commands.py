...
class GenerateAttitudeCommands():
  ...
  def generate_attitude_commands(self) -> None:
    test_angle_rad = np.deg2rad(2.5)
    # self.estimate_roll_models = False
    # self.estimate_pitch_models = True
    ...
    if self.estimate_models:
      if self.estimate_roll_models:
        ...
        roll = [0, test_angle_rad, 0, -test_angle_rad, 0]
      else:
        rospy.loginfo("Generating commands for pitch")
        pitch = [0, test_angle_rad, 0, -test_angle_rad, 0]
    ...
    while not rospy.is_shutdown():
      ...
      # if np.abs(attitude_cmd[0]) < 1e-6:
      #   attitude_cmd[0] = 0
      # if np.abs(attitude_cmd[1]) < 1e-6:
      #   attitude_cmd[1] = 0

      cmd_msg = AttitudeCommand()
      cmd_msg.header.stamp = rospy.Time.now()
      cmd_msg.roll = 0# attitude_cmd[0] 
      cmd_msg.pitch = 0# attitude_cmd[1]
      cmd_msg.yaw = 0
